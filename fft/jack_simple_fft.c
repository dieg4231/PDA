...
jack_port_t *input_port, *output_port, *buffer_a, *buffer_b;
int buffers_size;
...
int main (int argc, char *argv[]) {
	...
	jack_nframes_t buffer_calback_size=4096;
	hann(100);
	/*
	for(int i = 0 ; i < 100; i++)
		printf("%f\n",hann_values[i]);
	...
	*/
	...
	if( !jack_set_buffer_size(client,buffer_calback_size) )
		printf("Buffer size changed\n");
	else
		printf("Buffer size did not change\n");
