...
int jack_callback (jack_nframes_t nframes, void *arg)
{
	...
	for(i = 2, k = 0; i < 6; i++)
		for(j = 0; j < WINDOW_SIZE; ++j, ++k)
			buffer_b[k] = 0;//buffer_0[i][j] * hann_values[k];

    for(i = 512,j = 2560, k = 0 ; k < WINDOW_SIZE; ++i, ++j, ++k)
    	out[k] = in[k];//creal(buffer_b[i] + buffer_a[j]);		
...
int main (int argc, char *argv[]) {
	...
	buffer_a = ( double complex *) malloc( sizeof(double complex) * WINDOW_SIZE * 4 );
	buffer_b = ( double complex *) malloc( sizeof(double complex) * WINDOW_SIZE * 4 );
	buffer_0 = ( double complex **) malloc( sizeof(double complex *) * 6 );
	for(i = 0; i < 6 ; ++i)
...
void hann(int size )
{
	...
	hann_values = (double *)malloc(sizeof(double)*100);
	for ( i = 0; i < size; i++) 
